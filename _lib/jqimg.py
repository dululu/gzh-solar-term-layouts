...
六张插画的金线（#C9A227 山脊线）高度各不相同 —— 实测 gold_top 从 y=329 到 y=572，
山脊主体 gold_axis 从 y=388 到 y=695。所以裁切一律以「该张图自己的金线位置」为锚点，
写死 y 区间会让某些图只剩半座山。

注意模块命名：**不要给这个目录里任何一个 .py 起名 copy.py / json.py / types.py 之类**，
它会遮住同名标准库模块。历史上这里就有一个 `copy.py`（三套共用的演示正文），
在「先把 _lib 插进 sys.path 再 import PIL」的顺序下会让 PIL 挂在
`module 'copy' has no attribute 'deepcopy'`。已于 2026-09-19 改名 `demo_text.py`。
...
def gold_top(im, step=3, need=2):
    ...
    金黄色的，gold_top=360），或者一枚杂点（大雪的 gold_top=367，真山脊在 588）。
...
def gold_axis(im, step=3):
    ...
    两个真实踩过的坑（2026-09-19 用当前代码复核过，数字是实测值）：
      · 霜降的柿子枝一路垂到 y=545，柿子叶又是金黄色，gold_top 会被叶子骗到 360；
        真山脊的 argmax 在 695（22 个金色像素，和 760 / 764 并列）。
      · 大雪的 gold_top=367 是杂点，真山脊在 588（50 个金色像素，领先幅度很大）。

    **并列时取更靠上的那一行**（`cnt > best_c` 才更新，所以同分时先遇到的小 y 胜出）。
    这条规则是显式的、可复现的，但要注意它决定了霜降落在 695 而不是 760 —— 
    如果把「数金色」的阈值调松一点，霜降会在 695 / 760 之间跳 65px。
    改阈值或改 step 之后，务必六张图都复核一遍锚点。
    """
    w, h = im.size
    px = im.load()
