...
三套的正文、章节、色板都从 `_lib/demo_text.py` 读，**改一处三套同步**。
...
**换内容**：改 `_lib/demo_text.py`（标题、开场、六章、收束、落点动作），再重跑该套的 `build_article.py`。
...
_lib/
  ...
  demo_text.py           三套共用的演示正文（改内容只动这里）
...
但霜降那张的柿子枝梢是金黄色、大雪那张有杂点 —— 按「第一处金色」定位，霜降的锚点会被叶子
骗到 `y=360`、大雪被骗到 `y=367`，而真山脊分别在 **695** 和 **588**（实测 argmax），
裁出来的带里只剩天空。`jqimg.gold_axis()` 改成「逐行数金色像素取 argmax」：
山脊是横贯画面的长线，峰值行一定落在它身上。**同分时取更靠上的一行**（严格大于才更新），
这条要显式记着 —— 霜降的 695 和 760 / 764 就差 1 个像素，动阈值或动 `step` 会让它跳 65px。
...
**给「头图 / 结尾图」裁带子时，下沿要避开主体。** 正文里的窄带只要一条山脊就够，
但头带会被标题压着往下看，任何被切一半的主体都会露馅：

- 夏至的荷花开在源图 `y=503~690`，套二默认的 `down=120`（下沿 595）正好从花腰切断，
  黑花心贴着带子底边 —— 看起来就是个 bug。头图要 `down=225`（下沿 700），
  荷花整体收进带内、下方留 14px，再往下的荷叶从画面底边淌出去才自然。
- 霜降的柿子枝一路垂到 `y≈545`，而带子的上沿就是 `axis-150=545` ——
  硬边正好切在枝上，果子、黄叶、细枝全贴着顶边露出半截。**所以头图不能用霜降**，
  哪怕它的金线是六张里最好看的。挑头带要同时看「上沿能不能压到主体之下」和
  「下沿能不能兜住主体」。

**别再给 `_lib/` 里的文件起名 `copy.py`。** 三套共用的演示正文原本叫 `_lib/copy.py`，
它会遮住标准库的 `copy` 模块：只要有人先把 `_lib` 插进 `sys.path` 再 `import PIL`，
PIL 链路上的 `xml.etree` 就会挂在 `module 'copy' has no attribute 'deepcopy'`，
而报错信息完全看不出跟 `copy.py` 有关。2026-09-19 已改名 `_lib/demo_text.py`，
`json.py` / `types.py` / `io.py` 同理都别用。

**量页面高度必须等 `load` 事件。** `document.documentElement.scrollHeight` 如果在解析期
就读，`<img>` 还是 0 高度，量出来的页高会明显偏小（实测 3242 vs 真实 4359），
整页截图被截断在正文中途 —— 而且只有翻到最末尾才看得出来。写成
`window.addEventListener('load', ...)` 再写进 `document.title` 给 `--dump-dom` 读回来。
...
同色系压暗一档的值（见 `demo_text.py` 每章第 7 个字段 `文字色`），色点 / 色条展示时
雪白换成雪灰 `#CFD6DA`。
